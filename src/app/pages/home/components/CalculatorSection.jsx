...
export default function CalculatorSection() {
  ...
  const containerData = [
    { label: formatMessage({ id: 'app.pages.home.calculator.option.20ft_standard' }), payload: '28,200 KG', capacity: '33.2 CBM', scaleX: 1, scaleY: 1 },
    { label: formatMessage({ id: 'app.pages.home.calculator.option.40ft_standard' }), payload: '26,700 KG', capacity: '67.7 CBM', scaleX: 1.6, scaleY: 1 },
    { label: formatMessage({ id: 'app.pages.home.calculator.option.40ft_high_cube' }), payload: '26,500 KG', capacity: '76.3 CBM', scaleX: 1.6, scaleY: 1.2 },
  ];
        ...
        <div>
          ...
          <div className="space-y-8 md:space-y-12">
            <div>
              ...
              <input
                className="w-full h-3 bg-text-secondary rounded-full appearance-none cursor-pointer accent-primary"
                max="2"
                min="0"
                step="1"
                type="range"
              ...
              />
              <div className="flex text-text-primary justify-between mt-3 md:mt-4 text-xs opacity-50 uppercase tracking-widest font-label-sm">
                <span>{formatMessage({ id: 'app.pages.home.calculator.tick.20ft' })}</span>
                <span>{formatMessage({ id: 'app.pages.home.calculator.tick.40ft' })}</span>
                <span>{formatMessage({ id: 'app.pages.home.calculator.tick.40ft_hq' })}</span>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 md:gap-6">
              <div className="bg-card-secondary p-4 md:p-6 rounded-xl border border-outline-variant/30 flex flex-col gap-1">
                <div className="text-xs text-text-primary opacity-50 uppercase font-label-sm tracking-widest">{formatMessage({ id: 'app.pages.home.calculator.payload_max' })}</div>
                <div className="text-lg text-text-primary md:text-2xl font-bold tabular-nums">{containerData[sliderValue].payload}</div>
              </div>
              <div className="bg-card-secondary p-4 md:p-6 rounded-xl border border-outline-variant/30 flex flex-col gap-1">
                <div className="text-xs text-text-primary opacity-50 uppercase font-label-sm tracking-widest">{formatMessage({ id: 'app.pages.home.calculator.capacity' })}</div>
                <div className="text-lg text-text-primary md:text-2xl font-bold tabular-nums">{containerData[sliderValue].capacity}</div>
              </div>
            </div>
          </div>
        </div>
